...
visitation_1980_raw <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980")
visitation_1999_raw <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1999")
visitation_2018_raw <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/2018")

#unpack data
unpacked_1980 <- httr::content(visitation_1980_raw, as = "text", encoding = "UTF-8") 
unpacked_1999 <- httr::content(visitation_1999_raw, as = "text", encoding = "UTF-8")
unpacked_2018 <- httr::content(visitation_2018_raw, as = "text", encoding = "UTF-8")
...
visitation_ROMO_raw <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=12&endYear=2021")

visitation_ROMO_unpacked <- httr::content(visitation_ROMO_raw, as = "text", encoding = "UTF-8")

visitation_ROMO_final <- jsonlite::fromJSON(visitation_ROMO_unpacked)

visitation_ROMO_final
```
...
```{r}
unit_visitation <- function(unit_code, start_year, end_year, start_month, end_month){
  #pull in the data
  raw_unit_visitation <- httr::GET(url =
                                     paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unit_code, "&startMonth=", start_month, "&startYear=", start_year, "&endMonth=", end_month, "&endYear=", end_year))
  
  extracted_unit_visitation <- httr::content(raw_unit_visitation, as = "text", encoding = "UTF-8")
  
  final_unit_visitation <- jsonlite::fromJSON(extracted_unit_visitation)
  
  return(final_unit_visitation)
}

yellowstone <- unit_visitation(unit_code = "YELL", start_year = "1980", end_year = "2021", start_month = "01", end_month = "12")
...
```{r}
Rocky_mountain <- unit_visitation(unit_code = "ROMO", start_year = "1990", end_year = "2021", start_month = "01", end_month = "12")

Everglades <- unit_visitation(unit_code = "EVER", start_year = "1990", end_year = "2021", start_month = "01", end_month = "12")

Theodore <- unit_visitation(unit_code = "THRO", start_year = "1990", end_year = "2021", start_month = "01", end_month = "12")
```
...
```{r}
#Create function with default month values

unit_visitation_def <- function(unit_code, start_year, end_year, start_month = "01", end_month = "12"){
  #pull in the data
  raw_unit_visitation_def <- httr::GET(url =
                                     paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unit_code, "&startMonth=", start_month, "&startYear=", start_year, "&endMonth=", end_month, "&endYear=", end_year))
  
  extracted_unit_visitation_def <- httr::content(raw_unit_visitation_def, as = "text", encoding = "UTF-8")
  
  final_unit_visitation_def <- jsonlite::fromJSON(extracted_unit_visitation_def)
  
  return(final_unit_visitation_def)
}

#Return data using the default month values
Rocky_mountain_def <- unit_visitation_def(unit_code = "ROMO", start_year = "1990", end_year = "2021") #Don't need to put in the names for the parts of the function that have already been defined

Everglades_def <- unit_visitation_def(unit_code = "EVER", start_year = "1990", end_year = "2021")

Theodore_def <- unit_visitation_def(unit_code = "THEO", start_year = "1990", end_year = "2021")
```
...
```{r}
#Develop a list to store each year's output
...
for(i in 1:length(years)){ #for each one of the objects within our year vector
  # save parkwide data in output_floop. double brackets mean list in "i" list save "i" year output
  output_floop[[i]] <- parkwide_visitation(year = years[i]) #you want parkwide visitation to feed each year
  
}

str(parkwide_visitation)
```
...
```{r}
#create a vector listing each park unit
park_unit_ex6 <- c("ROMO", "EVER", "THRO")

#create output empty list by park unit
output_ex6_floop <- vector("list", length = length(park_unit_ex6))

#create for loop
for(i in 1:length(park_unit_ex6)){
  
  output_ex6_floop[[i]] <- unit_visitation(unit_code = park_unit_ex6[i], start_month = "01", end_month = "12", start_year = "1980", end_year = "2021")
  
}

# see summary output of for loop
summary(output_ex6_floop)

#bind three dataframes to create one dataframe
multi_units_ex6 <- dplyr::bind_rows(output_ex6_floop)
```
...
output_map_ex7 <- park_unit_ex6 %>%
  map(~ unit_visitation(unit_code = ., start_month = "01", end_month = "12", start_year = "1980", end_year = "2021"))

multi_units_ex7 <- bind_rows(output_map_ex7)
```
